...
<div class="max-w-4xl mx-auto">
    <div class="glass-card p-8 rounded-2xl">
        <div class="mb-6">
            ...
            <p class="text-gray-300">Add a new child to the ministry</p>
        </div>

        <form method="POST" action="{{ route('children-ministry.store') }}" class="space-y-6">
            @csrf

            <div class="grid grid-cols-2 gap-6">
                <div class="col-span-2">
                    <label class="block text-sm font-semibold text-gray-300 mb-2">Child Name *</label>
                    <input type="text" name="child_name" required class="input-field" value="{{ old('child_name') }}">
                    @error('child_name')
                ...
                <div>
                    ...
                    <input type="date" name="dob" required class="input-field" value="{{ old('dob') }}">
                    @error('dob')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="parent_name" required class="input-field" value="{{ old('parent_name') }}">
                    @error('parent_name')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="contact" required class="input-field" value="{{ old('contact') }}">
                    @error('contact')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="class_group" class="input-field">
                        ...
                        <option value="Nursery" {{ old('class_group') == 'Nursery' ? 'selected' : '' }}>Nursery</option>
                        <option value="Pre-teens" {{ old('class_group') == 'Pre-teens' ? 'selected' : '' }}>Pre-teens</option>
                        <option value="Teens" {{ old('class_group') == 'Teens' ? 'selected' : '' }}>Teens</option>
                    </select>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-gray-300 mb-2">Registration Date *</label>
                    <input type="date" name="registered_on" required class="input-field" value="{{ old('registered_on', date('Y-m-d')) }}">
                    @error('registered_on')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="allergies" rows="2" class="input-field" placeholder="Any allergies or health conditions...">{{ old('allergies') }}</textarea>
                </div>
