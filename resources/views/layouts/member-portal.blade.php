    ...
    <div class="flex min-h-screen relative">
        ...
        <aside class="sidebar" id="sidebar" w-80 h-screen fixed left-0 top-0 z-40 flex flex-col overflow-hidden">
            ...
            <nav class="p-6 space-y-2 flex-1 sidebar-nav" style="overflow-y: scroll !important; overflow-x: hidden !important; max-height: calc(100vh - 250px); min-height: 400px;">
                ...
                <a href="{{ route('portal.attendance') }}" class="sidebar-item {{ request()->routeIs('portal.attendance') ? 'active' : '' }} flex items-center text-green-300 hover:text-green-200 px-6 py-4 rounded-2xl group relative z-10">
                    <div class="icon-box w-12 h-12 flex items-center justify-center rounded-xl {{ request()->routeIs('portal.attendance') ? 'gradient-blue' : 'bg-white/5' }} group-hover:gradient-blue transition-all relative z-10">
                        <i class="fas fa-calendar-check text-white"></i>
                    </div>
                    <span class="ml-4 font-semibold text-sm relative z-10">My Attendance</span>
                    <i class="fas fa-chevron-right ml-auto text-xs opacity-0 group-hover:opacity-100 transition-opacity relative z-10 text-green-300"></i>
                </a>
                
                <a href="{{ route('qr.service.scanner') }}" class="sidebar-item {{ request()->routeIs('qr.service.scanner') ? 'active' : '' }} flex items-center text-green-300 hover:text-green-200 px-6 py-4 rounded-2xl group relative z-10">
                    <div class="icon-box w-12 h-12 flex items-center justify-center rounded-xl {{ request()->routeIs('qr.service.scanner') ? 'gradient-green' : 'bg-white/5' }} group-hover:gradient-green transition-all relative z-10">
